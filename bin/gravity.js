...
async function showHelp() {
  clack.intro("Welcome to the Schoolflow Toolbelt 🔧");
  ...
  const action = await clack.select({
    ...
    options: [
      {
        value: "view",
        label: "Scaffold new MVC",
        hint: "New set of model, view, controller and API files",
      },
...
async function createMVC(spinner) {
  ...
  const options = await clack.multiselect({
    message:
      "Want anything extra? (press space to select, enter to confirm or skip)",
    options: [
      {
        value: "-ui",
        label: "Create the UI view",
        hint: "Add the UI view and route to the React client",
      },
      {
        value: "-db",
        label: "Create the DB migration",
        hint: "Create a new migration file to setup your database table",
      },
    ],
    required: false,
  });
  ...
  if (err) return clack.log.error(err.message);
...
async function createMasterAccount(spinner) {
  ...
  const { err } = await master.create(
    ["create", "master", `${email}:${password}`],
    true
  );
